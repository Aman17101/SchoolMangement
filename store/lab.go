...
func (store Postgress) CreateLab(lab *model.Lab) error {
	...
	if response.Error != nil {
		util.Log(model.LogLevelError, model.StorePackage, model.CreateLab, "err while creating new lab", response.Error)
		return response.Error
	}
	util.Log(model.LogLevelInfo, model.StorePackage, model.CreateLab, "successfully created lab ", lab)
	return nil
}
...
func (store Postgress) GetLabs() ([]model.Lab, error) {
	labs := []model.Lab{}
	util.Log(model.LogLevelInfo, model.StorePackage, model.GetLabs, " fetching record for lab from db ", nil)

	if err := store.DB.Find(&labs).Error; err != nil {
		util.Log(model.LogLevelError, model.StorePackage, model.GetLabs, "err while fetching labs from db", err)
		return labs, err
	}
...
func (store Postgress) GetLab(labID uuid.UUID) (model.Lab, error) {
	...
	util.Log(model.LogLevelInfo, model.StorePackage, model.GetLab, " fetching record for lab from db ", nil)
	var lab model.Lab
