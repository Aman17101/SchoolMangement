...
func (store Postgress) CreateRoom(room *model.Room) error {
	...
	if response.Error != nil {
		util.Log(model.LogLevelError, model.StorePackage, model.CreateRoom, "err while creating new room", response.Error)
		return response.Error
	}
	util.Log(model.LogLevelInfo, model.StorePackage, model.CreateRoom, "successfully created room ", room)
	return nil
}
...
func (store Postgress) GetRooms() ([]model.Room, error) {
	rooms := []model.Room{}
	util.Log(model.LogLevelInfo, model.StorePackage, model.GetRooms, " fetching record for room from db ", nil)

	if err := store.DB.Find(&rooms).Error; err != nil {
		util.Log(model.LogLevelError, model.StorePackage, model.GetRooms, "err while fetching rooms from db", err)
		return rooms, err
	}
...
func (store Postgress) GetRoom(roomID uuid.UUID) (model.Room, error) {
	...
	util.Log(model.LogLevelInfo, model.StorePackage, model.GetRoom, " fetching record for room from db ", nil)
	var room model.Room
