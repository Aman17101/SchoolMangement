...
func (store Postgress) CreateClass(class *model.Class) error {
	...
	if response.Error != nil {
		util.Log(model.LogLevelError, model.StorePackage, model.CreateClass, "err while creating new class", response.Error)
		return response.Error
	}
	util.Log(model.LogLevelInfo, model.StorePackage, model.CreateClass, "successfully created class ", class)
	return nil
}
...
func (store Postgress) GetClasss() ([]model.Class, error) {
	classs := []model.Class{}
	util.Log(model.LogLevelInfo, model.StorePackage, model.GetClasss, " fetching record for class from db ", nil)

	if err := store.DB.Find(&classs).Error; err != nil {
		util.Log(model.LogLevelError, model.StorePackage, model.GetClasss, "err while fetching classs from db", err)
		return classs, err
	}
...
func (store Postgress) GetClass(classID uuid.UUID) (model.Class, error) {
	...
	util.Log(model.LogLevelInfo, model.StorePackage, model.GetClass, " fetching record for class from db ", nil)
